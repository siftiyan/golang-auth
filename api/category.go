...
func (ct *categoryAPI) UpdateCategory(c *gin.Context) {
	...
	// kita olah datanya - validasi (ini sudah diatas itu)
	// terus kita update
	err := ct.categoryService.Update(idInt, updateCat)
	if err != nil {
		c.JSON(http.StatusInternalServerError, model.ErrorResponse{Error: err.Error()})
		return
	}
	...
	c.JSON(http.StatusOK, model.NewSuccessResponse("category update succes"))
}
